...
module.exports = {
    category: 'All',
    guildOnly: true,
    description: 'Create a customizable embed', // Required for slash commands
    slash: true,
    testOnly: false,
    callback: async ({ interaction, client }) => {
        ...
        commandChannel.send({
            content: `${getDate()} COMMAND \`/embed\` WAS EXECUTED BY ${interaction.user.username}`
        })
        ...
        async function getPermitedRoles() {
            let query = await con2.execute(`SELECT EXISTS (SELECT role_id FROM verify WHERE server_id = '${serverid}' AND command = 'embed') AS result;`);
            query = query[0][0].result
            if (query == 0) {
                return [false];
            } else {
                let query = await con2.execute(`SELECT role_id FROM verify WHERE server_id = '${serverid}' AND command = 'embed'`);
                let verifyArr = []
                            ...
                            if (collected2.first().content.toLowerCase() == 'cancel') {
                                ...
                                if (isHex == false) {
                                    interaction.followUp({
                                        content: `:x: Invalid Hex Code!`, allowedMentions: { parse: [] }
                                    })
                                    return;
                                }
                                ...
                                const embed = new MessageEmbed()
                                    ...
                                    .setColor('BLUE');
                                interaction.followUp({ embeds: [embed], fetchReply: true, allowedMentions: { parse: [] } }).then(channel.awaitMessages({ filter, max: 1, time: 180_000, errors: [] }).then(collected4 => {
                                        ...
                                        if (collected4.first().content.toLowerCase() == 'cancel') {
                                            ...
                                            interaction.followUp({ embeds: [embed], fetchReply: true, allowedMentions: { parse: [] } }).then(channel.awaitMessages({ filter, max: 1, time: 30_000, errors: [] }).then(collected6 => {
                                                        ...
                                                        if (collected6.first().content.includes('<#')) {
                                                            ...
                                                            if (guild.channels.cache.get(check)) {
                                                                ...
                                                                const embed = new MessageEmbed()
                                                                    ...
                                                                    .setColor(color)
                                                                    .setDescription(desc)
                                                                ch.send({ embeds: [embed], allowedMentions: { parse: [] } }).catch((err) => {
                                                                    return interaction.followUp({
                                                                        content: `Unkown Error Occured. Please Check If I have permission to send in the mentioned channel`, allowedMentions: { parse: [] }
                                                                    })
                                                                })
                                                                interaction.followUp({
                                                                    content: `Embed Has successfully sent to the mentioned channel.`, allowedMentions: { parse: [] }
                                                                })
                                                                return;
                                                            } else {
                                                                interaction.followUp({
                                                                    content: `This channel does not exits on this server.`, allowedMentions: { parse: [] }
                                                                })
                                                            }
                                                        } else {
                                                ...
                                                interaction.channel.send({
                                                    content: `Time is up!`, allowedMentions: { parse: [] }
                                                });
                                            });
                                        }
                                    }
